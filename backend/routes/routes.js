...
router.patch('/loja/:id', async (req, res) => {
    const id = req.params.id
    const { DT, DTCC, DTCCFR, Distrito, Concelho, Freguesia, Morada, CodigoPost, Localidade, Cadeia, Insignia, DOP, Nome, CodigoLoja, AreaVenda, AnoAbertur, Lat, Long, Disponivel, Nivel_risco, dataAlgoritmo} = req.body
    const loja = {
      DT,
      DTCC,
      DTCCFR,
  ...
  router.delete('/loja/:id', async (req, res) => {
    ...
    try {
        const deleteloja = await lojaDataCopy.deleteOne({"req.params.lojaDataCopy_id": id })
